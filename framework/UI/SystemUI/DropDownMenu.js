...
class DropDownMenu extends BaseView {
  ...
  startDrawing(parent) {
    ...
    <div
      ...
      >
        ...
        <div
          ...
          "
        >
          <div class="${this.name}-title">level1</div>
        </div>
        <svg
          class="lowerArrow"
          style="
    ...
    styleTag.textContent = `
      #${this.name}:hover .menu {
        box-sizing: border-box;
        display: block !important;
        transform: scaleY(1) !important;
      }
      #${this.name}:hover .lowerArrow {
        display: none;
      }
    ...
    const svgs = this.element.querySelectorAll("._3-5");

    menuItems.forEach((item) => {
      item.addEventListener("click", () => {
        const targetIndex = item.getAttribute("data-target");

        document.querySelector(`.${this.name}-title`).innerText =
          item.innerText;
        ...
        // 显示对应的 SVG
        const targetSVG = document.querySelector(`[data-svg="${targetIndex}"]`);
